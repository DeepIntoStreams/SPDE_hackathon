...
from model.NORS.models import NORS2D  # noqa: E402
from model.NORS.features2d import build_mfv_2d, parabolic_graph_2d  # noqa: E402
from model.NORS.utils import LpLoss, count_params, load_spde_2d, make_splits  # noqa: E402
...
@hydra.main(version_base=None, config_path="../config", config_name="nors_2d")
def main(cfg: DictConfig) -> None:
    ...
    data = load_spde_2d(cfg.data_path, max_t=cfg.max_t, sub_t=cfg.sub_t, sub_x=cfg.sub_x)
    graph = parabolic_graph_2d(
        data,
        height=cfg.height,
    ...
    print(
        f"W: {data['W'].shape}, sol: {data['sol'].shape}, "
        f"X: {data['X'].shape}, Y: {data['Y'].shape}, T: {data['T'].shape}"
    )
