...
def mytrain(config):
    ...
    if config.load_cp:
        model.load_state_dict(torch.load(config.load_cp))
        print('Load checkpoint from:', config.load_cp)
    ...
    # plot_2d_xi(model, test_loader, device, T=config.T // config.sub_t)
