...
class Phi43H5NSPDEDataset(Dataset):
    """Lazy Phi43 HDF5 dataset for NSPDE.
    ...
    def __init__(self, h5_path: str, max_t: int | None = None, sub_t: int = 1, sub_x: int = 1):
        ...
        with h5py.File(self.h5_path, "r") as h5:
            ...
            self.x_indices = np.arange(0, nx, sub_x, dtype=np.int64)
            self.y_indices = np.arange(0, ny, sub_x, dtype=np.int64)
            self.z_indices = np.arange(0, nz, sub_x, dtype=np.int64)
            self.nx = len(self.x_indices)
            self.ny = len(self.y_indices)
            self.nz = len(self.z_indices)
    ...
    def __getitem__(self, index):
        ...
        w_txyz = w_txyz[:, self.x_indices][:, :, self.y_indices][:, :, :, self.z_indices]
        u_txyz = u_txyz[:, self.x_indices][:, :, self.y_indices][:, :, :, self.z_indices]
...
def relative_l2_loss(myloss, pred, target, ignore_t0=True):
    ...
    return myloss(pred.reshape(batch_size, -1), target.reshape(batch_size, -1))


def eval_nspde3d(model, loader, device, myloss, ignore_t0=True):
    model.eval()
    total = 0.0
    ...
    with torch.no_grad():
        for u0, xi, u in loader:
            ...
            u = u.to(device)
            pred = model(u0, xi)
            total += relative_l2_loss(myloss, pred, u, ignore_t0=ignore_t0).item()
...
def plot_loss_curve(loss_epochs, train_losses, val_losses, save_dir):
    ...
    plt.title("NSPDE Phi43 L2 Loss", fontsize=22, fontweight="bold")
...
def train_model(args):
    ...
    dataset = Phi43H5NSPDEDataset(args.data_path, max_t=args.max_t, sub_t=args.sub_t, sub_x=args.sub_x)
    ...
    for ep in range(args.epochs):
        ...
        val_loss = eval_nspde3d(model, val_loader, device, myloss, ignore_t0=not args.include_t0)
        test_loss = eval_nspde3d(model, test_loader, device, myloss, ignore_t0=not args.include_t0)
        ...
        if ep % args.print_every == 0 or ep == args.epochs - 1 or should_stop:
            train_loss = eval_nspde3d(model, train_eval_loader, device, myloss, ignore_t0=not args.include_t0)
    ...
    loss_train = eval_nspde3d(model, train_eval_loader, device, myloss, ignore_t0=not args.include_t0)
    loss_val = eval_nspde3d(model, val_loader, device, myloss, ignore_t0=not args.include_t0)
    loss_test = eval_nspde3d(model, test_loader, device, myloss, ignore_t0=not args.include_t0)
...
def main():
    parser = argparse.ArgumentParser(description="Train NSPDE fixed-point model on Phi43 HDF5 data")
    ...
    parser.add_argument("--wandb-name", type=str, default="NSPDE_phi43")
    parser.add_argument("--best-ckpt-name", type=str, default="NSPDE_phi43_best_val.pt")
