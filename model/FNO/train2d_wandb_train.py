...
def run_training_xi(data_path, ntrain, ntest, batch_size, epochs, learning_rate,
    ...
    plot_2d_xi(model, test_loader, device, wb=wb)
...
def main(cfg: DictConfig):
    ...
    config = OmegaConf.to_container(
        cfg.run.model, resolve=True, throw_on_missing=True
    )
    run = wandb.init(
    ...
    combined_args = {**cfg.run.data, **cfg.run.model, **cfg.run.save}
    run_training_xi(**combined_args, wb=run)
