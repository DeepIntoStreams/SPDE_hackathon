...
def make_seed_config(config, seed):
    ...
    base_best_ckpt_name = getattr(config, "best_ckpt_name", "fno_phi42_best_val.pt")
...
def collect_fno_predictions(model, data_loader, device):
    model.eval()
    reals, preds = [], []
    with torch.no_grad():
        for xi_, u_ in data_loader:
            xi_ = xi_.to(device)
            u_pred = model(xi_)
...
def evaluate_phi42_metrics(model, data_loader, device, prefix="test", include_rel_l2=True, verbose=True):
    u_real, u_pred = collect_fno_predictions(model, data_loader, device)
...
def measure_inference_time_fno(model, data_loader, device, warmup_batches=1):
    ...
    with torch.no_grad():
        for batch_idx, (xi_, _) in enumerate(data_loader):
            xi_ = xi_.to(device)
            if device.type == 'cuda':
...
def plot_loss_curve(loss_epochs, train_losses, val_losses, save_dir):
    ...
    plt.title('FNO Phi42 L2 Loss', fontsize=22, fontweight='bold')
...
def plot_metric_curve(metric_history, save_dir):
    ...
    plt.title('FNO Phi42 Validation Metrics', fontsize=22, fontweight='bold')
    plt.xlabel('Epoch', fontsize=18, fontweight='bold')
...
def train(config):
    ...
    init_wandb(config, default_project="FNO_Phi42")
    ...
    metric_history = []
    best_ckpt_name = getattr(config, "best_ckpt_name", "fno_phi42_best_val.pt")
    ...
    model, losses_train, losses_val = train_fno_2d(
    ...
    inference_stats = measure_inference_time_fno(model, test_loader, device)
...
def hyperparameter_search(config):
    ...
    if best_state_dict is not None:
        print('Best model kept in memory only:', best_config, 'best_val:', best_loss_val)
