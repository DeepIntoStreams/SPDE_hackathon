...
from model.FNO.train3d import (  # noqa: E402
    Phi43H5Dataset,
    eval_fno3d,
...
def make_eval_loader(args, data_path):
    dataset = Phi43H5Dataset(data_path, max_t=args.max_t, sub_t=args.sub_t)
...
def evaluate_one_dataset(args, model, data_path, dataset_idx, device, myloss):
    ...
    loss_test = eval_fno3d(model, loader, device, myloss, ignore_t0=not args.include_t0)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate FNO3D Phi43 checkpoint on independent HDF5 test datasets")
    ...
    parser.add_argument("--wandb-name", type=str, default="FNO_phi43_eval")
