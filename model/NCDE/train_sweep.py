...
def train(cfg):
    ...
    _, test_loader, _ = dataloader_ncde_1d(u=data, xi=xi,
                                           ntrain=ntrain + nval,
                                           ntest=ntest,
                                           T=wandb.config.T,
    ...
    train_loader, val_loader, normalizer = dataloader_ncde_1d(u=data[:ntrain + nval],
                                                              xi=xi[:ntrain + nval],
    ...
    model = NeuralCDE(input_channels=wandb.config.dim_x + 1,
    ...
    _, _, _ = train_ncde_wb(model, train_loader, val_loader, normalizer,
                            device, loss,
                            batch_size=wandb.config.batch_size,
    ...
    run.summary['loss_train'] = eval_ncde(model, train_loader, loss, wandb.config.batch_size, device,
                                          u_normalizer=normalizer)
    run.summary['loss_val'] = eval_ncde(model, val_loader, loss, wandb.config.batch_size, device,
                                        u_normalizer=normalizer)
    run.summary['loss_test'] = eval_ncde(model, test_loader, loss, wandb.config.batch_size, device,
                                         u_normalizer=normalizer)
