...
from model.DeepONet.deepOnet2D import *
...
def run_training_xi(data_path, ntrain, ntest, batch_size, epochs, learning_rate,
    ...
    train_loader, test_loader, normalizer, grid = dataloader_deeponet_2d_xi(u=data, xi=xi, ntrain=ntrain,
                                                          ntest=ntest, T=T, sub_t=sub_t, sub_x=sub_x,
                                                          batch_size=batch_size, normalizer=normalizer)

    dim_x = data.size(1)//sub_x
    S = dim_x * dim_x * (T//sub_t)
    ...
    model, losses_train, losses_test = train_deepOnet(model, train_loader, test_loader, grid,
...
def run_training_u0(data_path, ntrain, ntest, batch_size, epochs, learning_rate,
    ...
    train_loader, test_loader, normalizer, grid = dataloader_deeponet_2d_u0(u=data, ntrain=ntrain,
                                                          ntest=ntest, T=T, sub_t=sub_t, sub_x=sub_x,
                                                          batch_size=batch_size, normalizer=normalizer)
...
def hyperparameter_tuning(data_path, ntrain, nval, ntest, batch_size, epochs, learning_rate,
    ...
    data = torch.from_numpy(Sol.astype(np.float32))
    dim_x = data.size(1) // sub_x
    _, test_dl, norm, grid = dataloader_deeponet_2d_xi(u=data, xi=xi, ntrain=ntrain + nval,
                                                          ntest=ntest, T=T, sub_t=sub_t, sub_x=sub_x,
                                                          batch_size=batch_size, normalizer=normalizer)

    train_dl, val_dl, norm, grid = dataloader_deeponet_2d_xi(u=data[:ntrain + nval], xi=xi[:ntrain + nval],
                                                             ntrain=ntrain, ntest=nval, T=T, sub_t=sub_t, sub_x=sub_x,
                                                             batch_size=batch_size, normalizer=normalizer)

    S = dim_x * dim_x * (T // sub_t)
    hyperparameter_search_deeponet(train_dl, val_dl, test_dl, S, grid, norm,
...
@hydra.main(version_base=None, config_path="../config/", config_name="config_deeponet_NS_xi")
def main(cfg: DictConfig):
    print('Training deeponet on NS xi -> u...')
