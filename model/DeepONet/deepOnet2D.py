...
def dataloader_deeponet_2d_xi(u, xi, ntrain=1000, ntest=200, T=51, sub_t=1, sub_x=4, batch_size=20, normalizer=128,
    ...
    u_train = u[:ntrain, ::sub_x, ::sub_x, :T:sub_t].reshape(ntrain, -1)
    xi_train = xi[:ntrain, ::sub_x, ::sub_x, :T:sub_t]
    dim_x = xi_train.shape[1]
    dim_t = xi_train.shape[-1]
    if not conv:
        xi_train = xi_train.reshape(ntrain, -1)

    u_test = u[-ntest:, ::sub_x, ::sub_x, :T:sub_t].reshape(ntest, -1)
    xi_test = xi[-ntest:, ::sub_x, ::sub_x, :T:sub_t]
...
def dataloader_deeponet_2d_u0(u, ntrain=1000, ntest=200, T=51, sub_t=1, sub_x=4, batch_size=20, normalizer=128,
    ...
    u0_train = u0_train.repeat([1,1,1,T//sub_t])
    if not conv:
    ...
    u0_test = u0_test.repeat([1,1,1,T//sub_t])
    if not conv:
...
def train_deepOnet(model, train_loader, test_loader, grid, u_normalizer, device, myloss, batch_size=20, epochs=5000,
                      learning_rate=0.001, scheduler_step=100, scheduler_gamma=0.5, print_every=20,
                      plateau_patience=None, plateau_terminate=None, checkpoint_file='checkpoint.pt'):
    grid = grid.to(device)
...
def hyperparameter_search_deeponet(train_dl, val_dl, test_dl, S, grid, u_normalizer=None, width=[128, 256, 512],
                                   branch_depth=[2, 3, 4], trunk_depth=[2, 3, 4], epochs=500, print_every=20, lr=0.025,
                                   plateau_patience=100, plateau_terminate=100, log_file='log_nspde',
                                   checkpoint_file='checkpoint.pt', final_checkpoint_file='final.pt'):
    ...
    for (w, bd, td) in hyperparams:
        ...
        _, _, _ = train_deepOnet(model, train_dl, val_dl, grid, u_normalizer, device, loss, batch_size=20,
                                    epochs=epochs, learning_rate=lr, scheduler_step=500, scheduler_gamma=0.5,
                                    print_every=print_every, plateau_patience=plateau_patience,
                                    plateau_terminate=plateau_terminate, checkpoint_file=checkpoint_file)
