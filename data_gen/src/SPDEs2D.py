...
class SPDE2D():
    ...
    def Time_step(self, W, X, Y, dW, dx, dy, dt, un, n):
        
        A = self.Matrix_A(dx, dt, len(X))
        B = self.Matrix_B(dy, dt, len(Y))
        F = self.Matrix_F(W, un, n, dW, dt)
    ...
    def Renormalization_Time_step(self, X, Y, dx, dy, dt, un, n):
        ...
        U = np.zeros((un.shape[0], un.shape[1], un.shape[2]))
        for i in range(un.shape[0]):
            U[i, :, :] = np.dot(A, un[i, :, :]) + np.dot(un[i, :, :], B) + self.mu_(un[i, :, :]) * dt + un[i, :, :]
    ...
    def Parabolic(self, W, T=None, X=None, Y=None, diff=True):
        ...
        for i in tqdm(range(1, len(T))):
            Solution[:, i, :, :] = self.Time_step(W, X, Y, dW, dx, dy, dt, Solution[:, i - 1, :, :], i-1)
    ...
    def Renormalization(self, W, T=None, X=None, Y=None, diff=True):
        ...
        # Solve equations in paralel for every noise/IC simultaneosly
        for i in tqdm(range(1, len(T))):
            Solution[:, i, :, :] = self.Renormalization_Time_step(X, Y, dx, dy, dt, Solution[:, i - 1, :, :], i-1) - (3 * Solution[:, i - 1, :, :] ** 2 * X_eps[:, i - 1, :, :] + 3 * Solution[:, i - 1, :, :] * Xsquare[:, i - 1, :, :] + Xcubic[:, i - 1, :, :]) * dt
