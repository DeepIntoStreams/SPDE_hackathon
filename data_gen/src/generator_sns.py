...
def navier_stokes_2d(a, w0, f, visc, T, delta_t=1e-4, record_steps=1, stochastic_forcing=None):
    ...
    if stochastic_forcing is not None:
        J1 = stochastic_forcing['truncation']
        J2 = stochastic_forcing['truncation']
        # initialise noise
        bj = get_twod_bj(delta_t, [J1, J2], a, stochastic_forcing['alpha'], w_h.device)
    ...
    for j in tqdm(range(steps)):
        ...
        if stochastic_forcing:
            M = w_h.shape[0]  # batch size
            if N1>J1:  # N1=64, J1=32
                dW, dW2 = get_twod_dW_revised(bj, stochastic_forcing['kappa'], M, w_h.device)
            else:  # N1=64, J1=64/128/256
                dW, dW2 = get_twod_dW(bj, stochastic_forcing['kappa'], M, w_h.device)
                if N1<J1:
                    dW = dW[:, ::J1//N1, ::J2//N2]  # trc = (J//N) * space resolution
