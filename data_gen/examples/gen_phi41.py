...
def simulator(a, b, Nx, s, t, Nt, truncation, sigma, fix_u0, num):
    ...
    print("Truncation degree:", truncation)
...
def main(cfg: DictConfig):
    ...
    os.makedirs(f"{cfg.save_dir}/", exist_ok=True)
    scipy.io.savemat(f"{cfg.save_dir}/{filename}", mdict={'X': O_X, 'T': O_T, 'W': W, 'sol': Soln_add})
    print(f"Saved to {cfg.save_dir}/{filename}")
